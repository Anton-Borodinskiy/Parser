...
# def main ():
#     soup = BeautifulSoup(html_doc, 'html.parser')
#     first_p = soup.find('p', {'class': 'card-description'}).text
#     print(first_p)
#
# main()
...
#         sum_articuls += int(articul.text.split(" ")[-1])
...
#
#     # Извлечение числовых значений артикулов и их суммирование
#     sum_articuls = sum(int(articul.text.split(":")[1].strip()) for articul in articuls)
#
#     print(f"Сумма артикулов: {sum_articuls}")  # Вывод результата
# main()
...
def main ():
 # Инициализация объекта BeautifulSoup
    ...
    k=[re.search(r'\b\d+',str(i))[0] for i in articuls]
    # Извлечение числовых значений артикулов и их суммирование
    sum_articuls =sum(map(int,k))
...
#
#     # Извлекаем значения атрибута alt и суммируем их
#     total_sum = sum(int(img['alt']) for img in img_tags if img.get('alt') and img['alt'].isdigit())
#
#     print(f"Сумма всех значений в атрибуте alt тега img: {total_sum}")
#
#
...
#     soup = BeautifulSoup(html_doc, 'html.parser')
#
#     # Находим все теги img, обладающие атрибутами alt и приводим их к int
#     img_tags = [int(img['alt']) for img in soup.select('img[alt]')]
#
#     # Суммируем значения атрибутов alt
...
def sum_even_length_ids():
    ...
    summs = 0
    for ps in p_tags:
        if len(ps.text.replace(" ", "")) % 2 == 0:
            summs += int(ps["id"]) + int(ps["class"][0])
...
#
#     for p in data:
#         if not len(re.findall(r'\S', p.text)) % 2:
#             global id_sum, class_sum
#             id_sum += int(p['id'])
#             class_sum += int(*p['class'])
#
#
# id_sum = class_sum = 0
#
