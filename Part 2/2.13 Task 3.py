...
def calculate_total_price(html: str) -> float:
    ...
    all_div = soup.find_all("div", attrs={"class":"book-card"})
    total =0
    for div in all_div:
        price = float(div.find('p', class_='count price').text.replace("Цена: $", "").strip())
        count = int(div.find('p', class_='count stock').text.replace("Количество на складе: ", "").strip())
        total += price*count
...
# counter = 0
#
# for i in zip(soup.find_all('p', 'count price'), soup.find_all('p', 'count stock')):
# 	price, amount = map(lambda x: float(x.text.replace('$', '').rsplit()[-1]), i)
