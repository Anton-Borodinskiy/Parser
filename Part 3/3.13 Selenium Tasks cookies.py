...
with webdriver.Chrome() as webdriver:
    ...
    for cookie in cookies:
        ...
        webdriver.refresh()
        if int(webdriver.find_element(By.ID,"age").text.split(":")[-1].strip()) < age:
            age = int(webdriver.find_element(By.ID,"age").text.split(":")[-1].strip())
            print(age)
            print(cookie)
        time.sleep(2)
    time.sleep(100)
...
#         webdriver.add_cookie(cookie)
#         webdriver.refresh()
#         age = int(webdriver.find_element(By.CSS_SELECTOR, '#age').text.split()[-1])
#         skills = len(webdriver.find_elements(By.CSS_SELECTOR, '#skills li'))
#         all_guys.append((age, skills, webdriver.get_cookie(cookie['name'])['value']))
#         webdriver.delete_all_cookies()
# print(sorted(all_guys, key=lambda x: (x[0], -x[1]))[0][-1])

#TASK3
with webdriver.Chrome() as webdriver:
    webdriver.get('https://parsinger.ru/methods/5/index.html')
    cookies = webdriver.get_cookies()
    all_a = webdriver.find_elements(By.TAG_NAME, "a")
    dict_cookie = {"a": "", "p": "", "exp": 0}
    for a in all_a:
        ...
        cookies = webdriver.get_cookies()
        for cookie in cookies:
            if int(cookie['expiry']) > dict_cookie["exp"]:
                dict_cookie["exp"] = int(cookie['expiry'])
                dict_cookie["a"] = href
                dict_cookie["p"] = webdriver.find_element(By.ID,"result").text
                print(dict_cookie)
        webdriver.back()
        time.sleep(2)
#EXAMPLE
...
with webdriver.Chrome() as webdriver:
    ...
    #EXAMPLE 1
    # result = sum(map(lambda x: int(x.get("value")), driver.get_cookies()))